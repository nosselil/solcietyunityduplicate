...
public class DisconnectionController : MonoBehaviour
{
    ...
    IEnumerator PollLocalPlayerExistence()
    {
        ...
        while (true)
        {
            Debug.Log("DISCONNECT: Local player still exists: playerSpawnedInScene " + playerSpawnedInScene + " localPlayerGO " + localPlayerGO);

            if (playerSpawnedInScene && localPlayerGO == null)
            {
