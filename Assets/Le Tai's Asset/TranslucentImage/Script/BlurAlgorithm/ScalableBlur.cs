...
public class ScalableBlur : IBlurAlgorithm
{
    ...
    Rect Crop(Rect src, Rect cropRegion)
    {
        var rect = src;
    ...
    )
    {
        ...
        var activeRegionRelative = Intersect(activeRegion, srcCropRegion);
        ...
        if (stepCount > 1)
        {
            ...
            propertyBlock.SetVector(ShaderID.CROP_REGION, RectUtils.ToMinMaxVector(Crop(srcCropRegion, activeRegionFirst)));
        ...
        activeRegionLast = stepCount > 1 ? activeRegionLast : Crop(srcCropRegion, activeRegionLast);
