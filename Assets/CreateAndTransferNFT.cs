...
public class CreateAndTransferNft : MonoBehaviour
{
  ...
  void OnEnable() { 
    ...
    public void MintNFTToWallet()
    {
        StartMinting(walletAddress, artworkName, artistName, imageUrl).Forget();
    }

    private async UniTask StartMinting(string walletAddress, string artworkName, string artistName, string imageUrl)
    {
        Loading.StartLoading();

        if (Web3.Account == null)
        {
            Debug.LogError("Wallet is not logged in. Please log in first.");
            return;
        }

        Solana.Unity.Wallet.PublicKey recipientPublicKey;
        try
        {
            recipientPublicKey = new Solana.Unity.Wallet.PublicKey(walletAddress);
        }
        catch (System.Exception ex)
        {
            Debug.LogError("Invalid wallet address: " + ex.Message);
            return;
        }
        ...
        var associatedTokenAccount = AssociatedTokenAccountProgram
            .DeriveAssociatedTokenAccount(recipientPublicKey, mint.PublicKey);
        ...
        {
            Debug.LogError("NFT minting failed! Reason: " + res?.Reason);
        }
