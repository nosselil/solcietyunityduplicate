...
namespace Privy
{
    internal class IframeHandler : IWebViewHandler
    {

        private readonly WebViewManager _webViewManager; // Reference to WebViewManager

        internal IframeHandler(WebViewManager webViewManager)
        {
        ...
        private void InjectIframe(string url)
        {
            ...
            window.addEventListener('message', function(event) {{
                ...
                if (event.origin === new URL(iframe.src).origin) {{
                    if(event.data === 'ready') {{
                        unityInstance.SendMessage('WebViewManagerBridge', 'OnWebViewReady', '');
                    }} else {{
                        let data = JSON.stringify(event.data);
                        unityInstance.SendMessage('WebViewManagerBridge', 'OnMessageReceived', data);
                    }}
                }} else {{
