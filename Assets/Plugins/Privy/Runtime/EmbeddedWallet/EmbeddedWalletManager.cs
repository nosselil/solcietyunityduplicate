...
namespace Privy
{
    internal class EmbeddedWalletManager
    {
        ...
        internal async Task<string> AttemptConnectingToWallet(InternalAuthSession authSession)
        {
            ...
            string walletAddress = await ConnectWallet(token, walletEntropy, true); //Attempt to recover the wallet
        ...
        internal async Task<string> ConnectWallet(string accessToken, WalletEntropy walletEntropy, bool attemptRecovery = false)
        {
            if (_embeddedWalletState is EmbeddedWalletState.Connected connectedState)
            {
                return connectedState.WalletAddress;
        ...
        private async Task<string> ConnectWalletWithoutLock(string accessToken, WalletEntropy walletEntropy, bool attemptRecovery)
        {
            if (_embeddedWalletState is EmbeddedWalletState.Connected connectedState)
            {
                return connectedState.WalletAddress;
            ...
            if (result is IframeResponseError errorResponse)
            {
                ...
                if (attemptRecovery && errorResponse.Error.Type == "wallet_not_on_device")
                {
                    ...
                    return await RecoverWalletThenTryConnecting(accessToken, walletEntropy); //This could throw an error, which would be bubbled to connect, and caught
                }
                else
                {
        ...
        internal async Task<string> RecoverWalletThenTryConnecting(string accessToken, WalletEntropy walletEntropy)
        {
            ...
            if (result is IframeResponseSuccess<RecoverWalletResponseData> walletResponse)
            {
                ...
                return await ConnectWalletWithoutLock(accessToken, walletEntropy, false); // No recovery on second attempt
            }
        ...
        internal async Task<RpcResponseData.IRpcResponseDetails> Request(WalletEntropy walletEntropy, ChainType chainType, int hdWalletIndex, RpcRequestData.IRpcRequestDetails request) {
