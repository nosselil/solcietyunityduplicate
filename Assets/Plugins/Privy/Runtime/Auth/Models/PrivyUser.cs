...
namespace Privy
{
    public class PrivyUser : IPrivyUser
    {
        ...
        public IEmbeddedEthereumWallet[] EmbeddedWallets
        {
            get
            {
                ...
                var embeddedWallets = LinkedAccounts.EmbeddedWalletAccounts()
                    .Select(account => account.CreateEmbeddedWallet(walletEntropy.Value, this._embeddedWalletManager))
                    .ToArray();
        ...
        public async Task<IEmbeddedEthereumWallet> CreateWallet(bool allowAdditional = false)
        {
            ...
            {
                PrivyEmbeddedWalletAccount walletWithHighestIndex = LinkedAccounts.EmbeddedWalletWithGreatestWalletIndexOrNull();
        ...
        private async Task<IEmbeddedEthereumWallet> CreatePrimaryEthereumWallet(bool throwErrorIfPrimaryWalletExists)
        {
            ...
            var existingSolanaAccount = LinkedAccounts.EmbeddedSolanaWalletAccounts().FirstOrDefault(account => account.WalletIndex == 0);
            string primaryWalletAddress = await _embeddedWalletManager.CreateEthereumWallet(accessToken, solanaAddress: existingSolanaAccount?.Address);
        ...
        private async Task<IEmbeddedEthereumWallet> CreateHdWallet(int hdWalletIndex)
        {
            ...
            var walletEntropy = LinkedAccounts.WalletEntropyOrNull() ?? throw new PrivyException.EmbeddedWalletException(
                "A user must have a primary wallet before creating a wallet at HD indices >= 1.",
                EmbeddedWalletError.CreateFailed
            );

            // Validate user's wallet count is within wallet limit
            int currentWalletCount = EmbeddedWallets.Length;
            if (currentWalletCount >= MAX_WALLETS_ALLOWED)
            {
                throw new PrivyException.EmbeddedWalletException(
                    $"Wallet Create Failed: Maximum number of wallets ({MAX_WALLETS_ALLOWED}) reached.",
                    EmbeddedWalletError.MaxWalletsCreated);
            }
            
            // If there is already an existing wallet with the specified HD index, no need to create it
            PrivyEmbeddedWalletAccount existingWalletWithHdIndex = LinkedAccounts.EmbeddedWalletByIndexOrNull(hdWalletIndex);
