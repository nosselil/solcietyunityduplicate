...
sampler2D _BackfaceEmissionMap;
...
half4 ApplyPolyboxFog(float height, half4 c)
{
	//Polybox Calculate heightFog
	half heightFog = saturate((_HeightFogEnd - height) / (_HeightFogEnd - _HeightFogStart));
...
#endif
#ifdef _HEIGHTFOG2
	half heightFog2 = saturate((_HeightFogEnd2 - height) / (_HeightFogEnd2 - _HeightFogStart2));
	heightFog2 = pow(heightFog2, 2.2 * _HeightFogPow2);
#ifdef _HEIGHTFOGCUTOFF
	heightFog2 *= step(height, _HeightFogStart);
#endif
...
half4 fragForwardBaseInternal (VertexOutputForwardBase i, fixed facing)
{
	FRAGMENT_SETUP(s)
#if UNITY_OPTIMIZE_TEXCUBELOD
	s.reflUVW		= i.reflUVW;
...
#ifdef _USEALBEDOFOG
	s.diffColor = ApplyPolyboxFog(i.eyeVec.w, s.diffColor.rgbb);
#endif
	...
	c.rgb += _BackfaceEmissionColor * tex2D(_BackfaceEmissionMap, i.tex.xy) * saturate(-facing);

#ifndef _USEALBEDOFOG
	c = ApplyPolyboxFog(i.eyeVec.w, c);
#endif
...
half4 fragForwardBase (VertexOutputForwardBase i, fixed facing : VFACE) : SV_Target	// backward compatibility (this used to be the fragment entry function)
{
